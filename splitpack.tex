...
Enrico, for creating the beautiful template
...
In this thesis, we will give a constructive proof that you can indeed pack all circle instances whose area does not exceed the combined area of the two circles shown in \Cref{fig:square-worst} into the unit square.

Summarize Split Packing in one sentence
...
\begin{table}
    ...
    \begin{tabular}{lcccc}
        \textbf{Shape} & \textbf{Worst case} & \textbf{Density} & \textbf{APX} & \textbf{APX} for equal circles\\
        Non-acute triangle & \begin{tikzpicture}\input{tikz/tri-table.tex}\end{tikzpicture} & $\sqrt{\frac{-(a-b-c)(a+b-c)(a-b+c)}{(a+b+c)^3}}\pi$ & d & e\\
        Thick isosceles triangle & \begin{tikzpicture}\input{tikz/iso-table.tex}\end{tikzpicture} & c & d & e\\
        Square & \begin{tikzpicture}\input{tikz/square-table.tex}\end{tikzpicture} & $\frac{\pi}{3+2\s} \approx 0.5390$& d & e\\
        Long rectangle & \begin{tikzpicture}\input{tikz/rect-table.tex}\end{tikzpicture} & $\frac{\pi a}{4b}$ & d & e\\
    \end{tabular}
\end{table}
