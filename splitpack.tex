...
\begin{lemma}\label{th:split-property}
    ...
    %    \item $b_1 \ge a_1 - $
    %\end{itemize}
    %. Set $a_i \coloneqq \mysum(C_i)$ and wlog assume $\frac{a_2}{f_2} \le \frac{a_1}{f_1}$.
    %Then $\min(C_1) \ge a_1 - f_1\frac{a_2}{f_2}$.
\end{lemma}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item[(2)]
            ...
            In \Cref{fig:hat-poke}, we display the situation when packing a hat into (w.l.o.g.) the right side of the container. $f_i$ is the relevant factor from the split key, $a_i$ is the hat's incircle and $y$ represents the hat's rounding.
...
\begin{proof}
    ...
    \begin{itemize}
        \item[(1)]
            The two gems don't overlap each other if their incircles don't overlap. We proved in \Cref{th:hats-in-hat} that this does not happen.
        \item[(2)]
...
%\begin{proof}
%    If $A$ only consists of a single $a$-circle, it can be placed at the center of the square, as its diameter of $2\sqrt{\frac{a}{\pi}} \approx 1.12838\sqrt{a}$ is smaller than the squares edge length $\sqrt{\Psi a} \approx 1.36207\sqrt{a}$. Otherwise, by \Cref{th:split-property}, \textsc{Split} decomposes $A$ into $X \in \C(x)$ and $Y \in \C(y,y-x)$. By \Cref{th:circlesinhat}, we can pack $X$ into a $x$-hat and $Y$ into a $(y,y-x)$-hat. By \Cref{th:hats-in-square}, we can pack those two hats into the square.
%
