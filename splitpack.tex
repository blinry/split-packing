...
\begin{table}
    ...
    \begin{tabular}{lcccc}
        ...
        Non-acute triangle & \begin{tikzpicture}[scale=2]\input{tikz/tri-table.tex}\end{tikzpicture} & $\sqrt{\frac{-(a-b-c)(a+b-c)(a-b+c)}{(a+b+c)^3}}\pi$\\
        Thick isosceles triangle & \begin{tikzpicture}[scale=2]\input{tikz/iso-table.tex}\end{tikzpicture} & c\\
        Square & \begin{tikzpicture}[scale=2]\input{tikz/square-table.tex}\end{tikzpicture} & $\frac{\pi}{3+2\s} \approx 0.5390$\\
        Long rectangle & \begin{tikzpicture}[scale=2]\input{tikz/rect-table.tex}\end{tikzpicture} & $\frac{\pi a}{4b}$\\
    \end{tabular}
\end{table}
...
\begin{theorem}
    Given a non-acute triangle with an incircle of area $a$, all circle instances with a combined area of up to $a$ can be packed into the triangle, and this area bound is tight.
\end{theorem}
...
\begin{theorem}
    ...
    All circle instances with a combined area of up to $a = \frac{\pi h}{4w}$ can be packed into the rectangle, and this area bound is tight.
\end{theorem}
...
\begin{proof}
    The triangle is an $(a,0)$-gem, and by \Cref{th:gem} we can pack any $R \in \R(a)$ into such a gem.
...
\begin{lemma}\label{th:square-worst}
    Two touching equal circles, packed into opposing corners of a squar, are the squares \emph{twincircles}, meaning that there are no two larger equal circles which can be packed.
\end{lemma}
...
\begin{proof}
    By \Cref{th:gems-in-square} and the Split Packing Theorem, the square is a $\R(a)$-shape. TODO naja
