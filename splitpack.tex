...
\begin{proof}
    ...
    So let us assume $a_i > f_i$. For this proof, we normalize the container hat's incircle $a$ to 1 to simplify calculations. Again, for hats similar to the container hat's right side, the ratio between the square root of its incircle's area and the length of its base is constant. We call this constant $d$.
    ...
    \begin{equation*}
        d\sqrt{a_i} - d\sqrt{b_i} + d\sqrt{f_i}\sqrt{b_i} \le d\sqrt{f_i}\\
    \end{equation*}

    We can divide by $d$ and factor out $\sqrt{b_i}$ to get:

    \begin{equation}\label{eq:tripoke}
        \sqrt{a_i} - (1-\sqrt{f_i})\sqrt{b_i} \le \sqrt{f_i}
    \end{equation}
    ...
    $$b_i \ge a_i - f_i\frac{a_j}{f_j} \ge a_i - f_i\frac{1-a_i}{1-f_i} = \frac{a_i(1-f_i)-f_i(1-a_i)}{1-f_i} = \frac{a_i-f_i}{1-f_i}$$

    Insert that into \Cref{eq:tripoke}:

    $$\sqrt{f_i+(a_i-f_i)} - (1-\sqrt{f_i})\frac{\sqrt{a_i-f_i}}{\sqrt{1-f_i}} \le \sqrt{f_i}$$

    Bring the subtrahend to the right and square both sides.

    $$f_i+(a_i-f_i) \le f_i + \frac{2\sqrt{f_i}(1-\sqrt{f_i})\sqrt{a_i-f_i}}{\sqrt{1-f_i}} + (1-b)^2\frac{(a_i-f_i)}{1-f_i}$$

    Subtract $f_i$ and divide by $\sqrt{a_i-f_i}$.

    $$\sqrt{a_i-f_i} \le \frac{2\sqrt{f_i}(1-\sqrt{f_i})}{\sqrt{1-f_i}} + (1-\sqrt{f_i})^2\frac{\sqrt{a_i-f_i}}{1-f_i}$$

    Rearrange,

    $$\sqrt{a_i-f_i}\frac{(1-f_i)-(1-\sqrt{f_i})^2}{1-f_i} \le \frac{(1-f_i)-(1-\sqrt{f_i})^2}{\sqrt{1-f_i}}$$

    Then multiply with $\sqrt{1-f_i}$:

    $$\sqrt{a_i-f_i}\frac{(1-f_i)-(1-\sqrt{f_i})^2}{\sqrt{1-f_i}} \le \frac{(1-f_i)-(1-\sqrt{f_i})^2}{\sqrt{1-f_i}}\sqrt{1-f_i}$$

    Divide by the large fraction:

    \begin{align*}
        &\sqrt{a_i-f_i} \le \sqrt{1-f_i}\\
        \iff &(a_i-f_i) \le 1-f_i\\
        \iff &a_i - f_i \le 1-f_i\\
        \iff &a_i \le 1\\
    \end{align*}

    As $a_i$ is never larger than 1, \Cref{eq:tripoke} is true and the hat always fits into the container. This completes the proof of \Cref{th:hats-in-hat}.
\end{proof}
