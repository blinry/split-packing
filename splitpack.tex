\documentclass[a4paper,style=print,oneside,bibliography=totoc,nexus,lnum,extramargin,hyperref]{tubsbook}
...
\title{Critical Densities for Packing Circles into Squares and Triangles}
...
\begin{lemma}\label{th:overlap}
    Place two circles of combined area $a$ in two corners of a triangle. Let $w$ be the length of the connecting side of the triangle. If the distance of the respective corner to the far point of the circle $i$'s projection onto the connecting side is $\sqrt{a_i}p_i$, then the two projections do not intersect if
