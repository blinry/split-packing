...
\textcite{GL1996repeated} indentify various regular patterns for packing equal circles into a square, which produce close-to-optimal results for instances where the number of circles equals certain forms like $k^2$, $k^2-1$, or $k(k+1)$.
...
There are two perspectives on the implications of the Split Packing theorem. Suppose that we have all three elements mentioned above in place so that we can use Split Packing to pack each circle instance into $\C(a)$ into a given shape.
