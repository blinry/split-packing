...
Already in 1967, \textcite{MM1967some} solved this problem for square objects. They proved that it is possible to pack a set of squares into the unit square, if their combined area does not exceed $1/2$. Their method first sorts the squares descending by size and then places them in the container square in a shelf-like manner. See the left of \Cref{fig:squares-worst} for an example packing. But at the same time, $1/2$ is the \emph{largest upper bound} you could hope for. Consider the packing in \Cref{fig:squares-worst}, consisting of two quarter-squares: If you made the squares only slightly larger, they could no longer be packed without overlapping each other.
...
\fig[5]{squares-worst}{Left: Example of Moon and Moser's shelf-packing. Right: Worst case for packing squares into a square.}

The equivalent problem for packing circles has since been open.
Let us call the largest upper bound $a^*$. When \textcite{DFL2010circle} posed \Cref{prb:1} in 2010, they already suggested that $a^*$ might be limited by the instance shown in \Cref{fig:square-worst}. Again, it is easy to argue that if these two circles were only a little larger, we could no longer pack them into the unit square without overlap. This means that their combined area represents an upper bound for $a^*$. 

\fig{square-worst}{Worst-case circle instance in terms of packing density?}

In this thesis, we will give a constructive proof that you can indeed pack all circle instances whose area does not exceed the combined area of the two circles shown in \Cref{fig:square-worst} into the unit square. The method recursively splits the circle instance in half and packs the two parts seperately.

Figure with an example packing
...
In \Cref{tab:results}, we summarize our results. Shown are the shape classes, a graphical depiction of the area that can always be packed, and a formula for the packing density of this worst case.

\begin{table}
    \caption{Overview of results.}\label{tab:results}
    \begin{tabular}{lcccc}
        \textbf{Shape} & \textbf{Worst case} & \textbf{Density}\\
        Non-acute triangle & \begin{tikzpicture}\input{tikz/tri-table.tex}\end{tikzpicture} & $\sqrt{\frac{-(a-b-c)(a+b-c)(a-b+c)}{(a+b+c)^3}}\pi$\\
        Thick isosceles triangle & \begin{tikzpicture}\input{tikz/iso-table.tex}\end{tikzpicture} & c\\
        Square & \begin{tikzpicture}\input{tikz/square-table.tex}\end{tikzpicture} & $\frac{\pi}{3+2\s} \approx 0.5390$\\
        Long rectangle & \begin{tikzpicture}\input{tikz/rect-table.tex}\end{tikzpicture} & $\frac{\pi a}{4b}$\\
    \end{tabular}
\end{table}
...
\textcite{FPT1981optimal} showed it to be NP-hard to pack equal squares into arbitrary polygons with holes by a reduction from \textsc{3-SAT}. They construct gadgets for variables and clauses and connect them with wires.% In Fig TODO, such a construction is depicted.
...
%TODO but not directly for circles: 5 x 1/5
...
While trying to solve \Cref{prb:1}, we worked on the assumption that it would in fact be possible to pack all circle instances whose area would not exceed that of the two-circle instance shown in \Cref{fig:square-worst}.%proposed by \textcite{DFL2010circle}.

%We attempted quite a few strategies to pack this area which did not work. For example, a shelf-packing based approach inspired by Moon \& Moser's algorithm for packing squares does not work for five equal circles, see TODO.
...
\paragraph{Approximation factor for unequal circles}
...
We saw in \Cref{ch:split-packing} that when applying \textsc{Split} to a circle instance, we will often get a group for which we can guarantee a minimum circle size.
If you think about how a circle of minimum size can be placed in the container, there might be certain points which can never be covered by a circle without the circle overlapping the container's boundary. For example, if the container's boundary contains angles, you cannot use all of the area inside of that angle, as the minimum circle might be too large to cover all of its points. As we cannot use such points for packing the circle instance, we might as well remove it from the container without changing its packing capabilities at all.
...
\begin{definition}
    ...
    Call the angle of the original triangle laying clockwise from the triangle's longest side the hat's \emph{left-angle}, and the one which lies counter-clockwise from the longest side the hat's \emph{right-angle}.
...
To prepare for the next proof, we need the following lemma:

\begin{lemma}\label{th:overlap}
    Place two circles of combined area $a$ in two corners of a triangle, like in \Cref{hats-overlap}. Let $w$ be the length of the connecting side of the triangle. If the distance of the respective corner to the far point of the circle $i$'s projection onto the connecting side is $\sqrt{a_i}p_i$, then the two projections do not intersect if

    $$w \ge \sqrt{a(p_1^2 + p_2^2)}.$$
\end{lemma}

\begin{proof}
    Let $w'(a_1) = \sqrt{a_1}p_1 + \sqrt{a-a_1}p_2$ be the combined width of both projections. This function has its global maximum at $a_1 = \frac{p_1^2}{p_1+p_2}a$, and the maximum value is $\sqrt{a(p_1^2+p_2^2)}$. If $w$ is at least as large as this value, the two projections do not intersect.
\end{proof}

\fig{hats-overlap}{text}

%\begin{lemma}\label{th:poke}
%    Take an $(a,0)$-hat with a left-angle of $\alpha$ as a container. Call the circles which define its split keys $f_1$ and $f_2$. Place a right $(a_1,b_1)$-hat with a right-angle of $\alpha$ into the container so that its tip touches the container's base, see TODO
%
%    The hat does not overlap the container if $a_1 \le f_1$ or $b_1 \ge a_1 - f_1\frac{a-a_1}{f_2}$
%\end{lemma}
...
\begin{proof}
    Place the hats' tips at the bottom of the container hat, rotate their $\alpha$- and $\beta$-angles towards the container's matching angles and push them as far to the left/right as possible. \Cref{tab:hats-in-hat} illustrates how these packings looks like for $a=1$ and different values of $a_1$ and~$a_2$.
    ...
    \begin{table}
        ...
        \begin{tabular}{cp{7cm}}
            ...
            \end{tikzpicture}
            & As $a_2$ gets smaller, the second hat gets less rounded, but still enough so that it can be packed. The first hat is a non-rounded triangle, which fits beside the second one.\\
            ...
            \end{tikzpicture}
            & When $a_1$ and $a_2$ have a ratio as specified by the container's associated split key, the two hats are both not rounded, but they still fit beside each other.\\
            ...
            \end{tikzpicture}
            & As $a_1$ approaches $a$, it is now the first hat whose shape gets closer to a perfect $a$-circle. By the same argument as before, this circle can be packed.\\
    ...
    These way of placing the two hats results in a valid packing if (1) the hats do not overlap each other and (2) the hats fit into the hat individually. We are going to proof these two properties separately.
    ...
    The second thing we need to show is that the hats fit into the container individually.
    ...
    So let us assume $a_i > f_i$. Again, for hats similar to the container hat's right side, the ratio between the square root of its incircle's area and the length of its base is constant. We call this constant $d$.
    ...
    We are also interested in the ratio between the square root of the area of the incircle of a triangle similar to the container triangle and the length of its right side. We call this ratio $e$. See \Cref{fig:hat-poke-f} for an illustration of $d$ and $e$. Let $o$ be the container's incircle.
    ...
    In \Cref{fig:hat-poke}, we display the situation when packing a hat into (\wlofg) the right side of the container. $f_i$ is the relevant factor from the split key, $a_i$ is the hat's incircle and $b_i$ represents the hat's rounding.
    ...
    \begin{equation}\label{eq:tripoke}
        \sqrt{a_i} + (\sqrt{f_i/o}-1)\sqrt{b_i} \le \sqrt{f_i}
    \end{equation}


    Let $j$ be the index of the other hat to be packed.
    We know that $a_i + a_j \le a$. Also, $f_i + f_j \ge a$ (TODO proof in a not yet existing section about worst cases in acute triangles).
    ...
    $$\sqrt{a_i} + (\sqrt{f_i/o}-1)\sqrt{a\frac{a_i-f_i}{a-f_i}} \le \sqrt{f_i}$$

    In our situation, the container's incircle $o$ has exactly the area $a$ we are trying to pack, so set $o$ to $a$. Also, bring the subtrahend to the right and square both sides.
    ...
    Divide by the large fraction:

    \begin{align*}
        &\sqrt{a_i-f_i} \le \sqrt{a-f_i}\\
        \iff &a_i - f_i \le a - f_i\\
        \iff &a_i \le a\\
    \end{align*}
...
Until now, the container was always a $(a,0)$-hat, which is essentially a non-rounded triangle with an incircle of $a$. The next lemma extends this idea to hats which are actually rounded. It is identical to \Cref{th:hats-in-hat}, except that that the rounding of the container hat is no longer 0, but $b$.
...
\begin{proof}
    ...
    Now assume that for any $0 \le b' \le a'$, an $(a',b')$-hat could pack all circle instances in $\C(a',b')$ with at most $n$ circles. Consider a circle instance $C$ containing $n+1$ circles.
    \Cref{def:hat-split-key} tells us how to compute the split key $F$.
    Then we know from \Cref{th:split-properties} that \textsc{Split} will partition $C$ into two subinstances $C_1 \in \C(a_1, b_1)$ and $C_2 \in \C(a_2, b_2)$, whose parameters are $(a,b,F)$-rounded. As \textsc{Split} can never return an empty instance (except for $|C| = 1$), each subinstance will contain at most $n$ circles. We know from \Cref{th:rounded-hats-in-hat} that, for all pairs of $(a,b,F)$-rounded tuples, we can find two hats with matching parameters which fit into the container hat. By assumption, these hats can now pack all instances from $\C(a_1, b_1)$ and $\C(a_2, b_2)$, respectively, which means that they can also pack $C_1$ and $C_2$ and that we can pack $C$ into the container hat.
...
\begin{theorem}
    A circle instance can be packed into a non-acute triangle if the circle's combined area does not exceed the area of the triangle's incircle.
\end{theorem}

\begin{proof}
    Call the incircle's area $a$. The triangle is an $(a,0)$-hat, which by \Cref{th:hats} is a $\C(a)$-shape.
\end{proof}

We also argue that this is the best area bound we could hope for:
...
\begin{proof}
    A single $b$-circle cannot be packed, as the incircle is by definition the largest circle which fits into the triangle.
\end{proof}
...
\begin{proof}
    ...
    When dilating the triangle by $r$, the remaining area is a triangle similar the the original one, whose longest side has a length of $2r$, see \Cref{fig:twincircles-construction}. This means that when dilating the triangle by $r+\varepsilon$, the longest side of the remaining area must be shorter than $2r$.
...
\begin{proof}
    ...
    Secondly, we show that the hats do not intersect the container. We know from \Cref{eq:tripoke} on \cpageref{eq:tripoke} that the following condition has to hold:

    $$\sqrt{a_i} + (\sqrt{f_i/o}-1)\sqrt{b_i} \le \sqrt{f_i}$$

    By construction TODO, $o \ge a$, so the above inequality is true if

    $$\sqrt{a_i} + (\sqrt{f_i/a}-1)\sqrt{b_i} \le \sqrt{f_i}$$

    And this is the same inequality that was shown to be true in \Cref{th:hats-in-hat}. The preconditions $a_i + a_j \le a$ and $f_i + f_j \ge a$ are also true here. This concludes the proof.

    %%Here, $f_i = a/2$, $o $ and $b_i = 2(a_i-f_i)$, which gives us:
    %In our case, $f_1 + f_2$ is exactly the twincircle area as constructed above, and $f_1 = f_2$, so $f_i = a/2$. The incircle of the triangle is larger than $a$ (TODO), so $o \ge a$. The hat's rounding is $b_i = a_i - f_i \frac{a-a_i}{f_i} = 2a_i-a$. So in total, we know the above condition to be true if

    %%$$\sqrt{a_i}-\sqrt{2-\sqrt{3}}\sqrt{a_i-f_i} \le \sqrt{f_i}$$
    %$$\sqrt{a_i}-(1-\sqrt{1/2})\sqrt{2a_i-a} \le \sqrt{a/2}$$

    %We square both sides:

    %%$$a_i+(2-\sqrt{3})(a_i-f_i) - 2\sqrt{a_i(2-\sqrt{3})(a_i-f_i)} \le f_i$$
    %$$a_i + x$$

    %Rearrange and square both sides again:

    %$$(a-f)^2(3-\sqrt{3})^2 \le 4a(2-\sqrt{3})(a-f)$$

    %And simplify to arrive at:

    %$$a_i \le 3f_i$$

    %As $a_i \le a = 2f_i \le 3f_i$, the above condition is true and the hats do not overlap the container. This concludes the proof.
\end{proof}

\begin{theorem}
    Any circle instance with a combined area of at most the triangle's twincircle area can be packed into a thick isosceles triangle.
\end{theorem}
...
What is the aspect ratio of the rectangle's sides where the combined area of the two circles incribed in the halves is exactly equal to the incircle area? In \Cref{fig:rect-construction}, we see that the area of the incircle is $A = \pi(\frac{a}{2})^2$.

\fig[4]{rect-construction}{Constructing the case where $A = 2B$.}

The area of a triangle with side lengths $a$, $b$, and $c$ can be calculated using Heron's formula:
...
So the area of the triangle's incircle can be calculated to be

$$I(a,b,c) := \pi R(a,b,c)^2 = \frac{(a+b-c)(c+a-b)(b+c-a)}{4(a+b+c)}$$

So in our case, $A = 2B$ is equivalent to

$$\frac{\pi}{4}a^2 = 2I(a,b,\sqrt{a^2+b^2}) = \frac{\pi ab(a+b-\sqrt{a^2+b^2}}{a+b+\sqrt{a^2+b^2}}$$

and equation this is true for $b = \frac{2+3\s}{4}a \approx 1.5607a$.
...
From this point forward, we will no longer pack circles, but rubies. To simplify talking about these instances, we make the following definition:

\begin{definition}
    A \emph{ruby instance} is defined excactly like a circle instance (see \Cref{def:circle-instance}), except that the real numbers now define the areas of the rubies' incircles.
\end{definition}
...
\begin{proof}
    Assume \wlofg\ that the two gems are an $(a_1, b_1)$-gem and an $(a_2, b_2)$-gem with $a_1 \ge a_2$, meaning that we always swap the larger gem to the left. Also, let us normalize $a$ to 1 to make calculations easier. The proofs hold for all $a$, one just needs to multiply all areas with $a$ and all lengths with $\sqrt{a}$.
...
Like with hats, “rounding” the container gem does not affect the packing properties:

\begin{lemma}\label{th:rounded-gems-in-gem}
    Any two $(a, b, (1,1))$-conjugated gems can be packed into an $(a,b)$-gem.
\end{lemma}

\begin{proof}
    \Cref{th:gems-in-gem} tells us that this theorem is true for $b = 0$. But when cutting all gems' corners by the same value, the inner gems will not overlap the container.
\end{proof}

\fig{gems-rounding}{Cutting all gems' corners by the same value does not affect the packing.}
...
\begin{theorem}
    Any ruby instance $R \in \C(a)$ can be packed into an isosceles right triangle with an incircle of area $a$.
\end{theorem}

\begin{proof}
    The triangle is an $(a,0)$-gem, and by \Cref{th:gem} we can pack any $R \in \C(a)$ into such a gem.
\end{proof}
...
\begin{proof}
    ...
    This placement constitues a valid packing because (1) the gems fit into the square individually and (2) the gems never overlap. This can be proven algebraically in exactly the same fashion as in \Cref{th:gems-in-gem}.
...
\begin{theorem}\label{th:square}
    Any ruby instance with a combined incircle area of $a$ can be packed into a square with twincircle area $a$.
\end{theorem}

\begin{proof}
    By \Cref{th:gems-in-square} and the Split Packing Theorem.
\end{proof}
