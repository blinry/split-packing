...
\begin{proof}
    ...
    To prepare for the next step, we need to make an observation about the factors $l$ and $r$ shown in \Cref{fig:hatlr}: If the top angle is a right angle, we can see that $lw = x$ and $rw = y$. So by Pythagoras, $(lw)^2 + (rw)^2 = w^2$, which is equivalent to $l^2 + r^2 \le 1$. If the top angle is more obtuse, but the incircle's center stays at the same $x$-coordinate (like the dotted variant in \Cref{fig:hatlr}), the incircle only gets smaller. This means that both $l$ and $r$ shrink, too, so for each hat, $l^2 + r^2 \le 1$, respectively $r \le \sqrt{1-l^2}$.

    \fig{hatlr}{$l^2 + r^2 \le 1$ holds for each non-acute triangle}

    If we enclose the incircles with hats which are \emph{similar} to the container, like in \Cref{fig:hats-overlap}, we see that the projections do not overlap if the following condition holds:
    ...
    Also note that for similar hats, the ratio between their incircle's area and the square of their widths is constant. This means that $a_1 + a_2 \le a$ (which is true) implicates $f^2 + g^2 \le w^2$. Thus, $g \le \sqrt{w^2-f^2}$.
