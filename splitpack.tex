...
\begin{theorem}[Split Packing]\label{th:splitpacking}
...
\begin{proof}
    ...
    Otherwise, use \textsc{Split}$(C,F)$ to produce two smaller subinstances $C_1$ and $C_2$. We know from \Cref{th:conjugated} that those those subinstances will always be $(a,b,F)$-conjugated. So if we can indeed find two shapes which pack these subinstances, and if we can pack these two shapes into $s$, then we can pack the original circle instance $C$ into $s$.

    %As these shapes also fit into $s$, we know that we can pack $C$ into $s$.
\end{proof}

As this theorem is important for the whole thesis, let us rephrase it: \Cref{th:splitpacking} describes what is sufficient to proof that a shape is a $\C(a,b)$-shape: Find a shape-specific split key $F$ and show that for all possible decompositions of the area $a$ (which is going to be perfomed by \textsc{Split}), that one can find $(F,b)$-rounded shapes of matching sums that fit into the original shape.
