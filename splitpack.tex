...
Split Packing also works for isosceles triangles.

\begin{lemma}
    Consider an isosceles triangle whose symmetric angles are not larger than $\pi/3$ and its twincircle area $a$.
    For any $b > a$ there are instances in $\C(b)$ which cannot be packed into the triangle.
\end{lemma}

\begin{proof}
    The instance consisting of two $b/2$-circles can not be packed
\end{proof}
...
\begin{lemma}\label{th:hats-in-iso}
    Consider an isosceles triangle whose symmetric angles are $\alpha$. Call its twincircle area $a$.
    For all $(a,0,F)$-conjugated tuples $(a_1, b_1)$ and $(a_2, b_2)$, the following two shapes can be packed into the hat:
    \begin{itemize}
        \item A right $(a_1,b_1)$-hat with a right-angle of $\alpha$ and
...
\begin{proof}
    First, we show that the hats do not intersect each other in a similar fashion as with hat containers: In a isosceles triangle, $p_1 = p_2$, so in \Cref{th:iso-width}, we construct the width of the triangle's base to be $2\sqrt{a/2}p_1 = \sqrt{2ap_1^2} = \sqrt{a(p_1^2+p_2^2)}$. By \Cref{th:overlap}, this means that the hats do not overlap.

    Second, we show that the hats do not intersect the container. We know from \Cref{eq:tripoke} on \cpageref{eq:tripoke} that the following condition has to hold:
...
\fig{iso-width}{The width of an isosceles triangle is x.}

\begin{theorem}
    %A circle instance can be packed into an isosceles triangle whose legs are at most as long as its base if the circle's combined area does not exceed the area of the triangle's incircle.
