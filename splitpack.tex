...
%In this thesis, we present a packing algorithm that solves problems like the following: \emph{What is the largest $a$ so that any set of circles with a combined area of up to $a$ can be packed into the unit square?}  Our results give sufficient conditions on the decicion problem of whether a given circle instance can be packed into a given container.
...
\begin{problem}[Critical density for packing circles in a square]\label{prb:1}
...
In \Cref{tab:results,tab:results2} we summarize our results. Each table entry represents a different packing problem, namely a specific combination of an object shape (like circles or squares), and a container shape (like a triangle or a recangle).
...
We then proceed to show how Split Packing can be applied to a number of different packing problems: In \textbf{\Cref{ch:circles}}, we discuss problems involving circular objects, and in \textbf{\Cref{ch:rubies}}, we show that the algorithm can even be generalized to pack objects other than circles.
...
%They construct gadgets for variables and clauses and connect them with wires.
...
\fig[4]{apollonian}{Circle instances which force a packing in the style of an Apollonian gasket approach a packing density of 1.}
...
While trying to prove the critical density for packing circles in a square, we worked on the assumption that it would in fact be possible to pack all circle instances whose area would not exceed that of the two-circle instance shown in \Cref{fig:worst-cases} on \cpageref{fig:worst-cases}.
...
\begin{definition}
    A shape's \emph{twincircles} are the largest two equal circles that can be packed into the shape.% see \Cref{fig:twincircles} for an example.% The shape's \emph{twincircle area} is their combined area.
\end{definition}
...
The method which we use to split the circle instances in half, \textsc{Split} (\Cref{alg:split}), resembles a greedy scheduling algorithm, which is why we call the process \emph{greedy splitting}. The algorithm takes two inputs: The first is an arbitrary circle instance $C$, which is to be split into two groups. The circles are assumbed to be sorted descendingly by size.
...
\begin{lemma}\label{th:min1}
    ...
    %For any circle instance $C$ and any split key $F = (f_1, f_2)$, let $C_1$ and $C_2$ be the products of \textsc{Split}$(C,F)$. If 
...
\begin{proof}
   % The condition is equivalent to $\mysum(C_i) - f_i\min\left\{\frac{\mysum(C_1)}{f_1},\frac{\mysum(C_2)}{f_2}\right\} > \frac{\mysum(C_i)}{2}$.
    By \Cref{th:min1},
...
%where the ratio between $a$ and the shape's area $ALG$ (we call the ratio $a/ALG$ the \emph{packing density}) is as high as possible.
...
If you think about how a circle of minimum size can be placed in a container, there might be certain points which can never be covered by a circle without overlapping the container's boundary. For example, if the container's boundary contains sharp angles, the minimum-size circle might be too large to cover all of the angle's area. As we can not use this for packing the circle instance at all, we might as well cut this area from the container without changing its packing capabilities.
...
\fig{hats-overlap}{The circles' projections do not overlap if $w \ge \sqrt{a(p_1^2 + p_2^2)}$.}
...
\begin{theorem}\label{th:iso}
    ...
    Expressed algebraically, for a triangle with legs of length $b$ and a base of length $c$, the density bound is
...
A class of triangles for which we could not prove the critical density are general acute triangles. The problem is that the condition for \Cref{th:overlap} is not met, which means that the two hats might overlap:
...
Unfortunately, this strategy fails for some instances, like depicted in \Cref{fig:four-fail}. For this instance, the largest group, consisting of a single circle, cannot be packed if \emph{any} of the smaller groups is packed in the top or the left corner, because the remaining free space is not wide enough for the circle. Other strategies or a case distincion would need to be considered.

\fig{four-fail}{This strategy does not work for a container with a right-angle of $\frac{\pi}{10}$, incircle $1$ and the circle instance $\{0.55,0.15,0.15,0.15\}$.}

\section{Rectangles}\label{sec:rectangles}

Some of our main results concern the packing of circles into a square. We will cover that packing problem in the next chapter, along with some generalizations on the objects which are to be packed, which is why we do not repeat it here. Instead, we first turn towards rectangles.
...
\begin{theorem}\label{th:rect}
    ...
    Expressed algebraically, the density bound is $\frac{\pi h}{4w}$.
\end{theorem}
...
\begin{proof}
    ...
    The $a$-circle's diameter is equal to the rectangle's smaller side, so it is clear that any larger circles can not be packed.
...
In fact it turns out that, if you are given a set of objects, no matter what their shape is, if you can enclose them in rubies so that the total area of the rubies does not exceed the critical density, then you can pack the shapes into these containers.
...
For this generalization, we need another new shape, which has a similar function as previous chapter's \emph{hats}. Our ruby shape has a very distinct way its corners are cut off. We generalize this cutoff to get to a family of shapes which call we \emph{gems}\footnote{A ruby is a special gem, get it?}:
...
\begin{definition}
    A \emph{sharp $(a,b)$-gem} is a isosceles triangle where only one corner is cut off like in the definition of a gem, see \Cref{fig:sharp-ruby}. A \emph{sharp ruby} is a sharp $(a,a)$-gem.
\end{definition}
...
\begin{proof}
    ...
    A sharp ruby with an incircle area of more than $a$ can not be packed, as its incircle would be larger than the triangle's incircle.
...
\begin{theorem}\label{th:square}
    ...
    Expressed algebraically, the density bound for the rubies' ares is
...
\begin{proof}
    ...
    The density bound can be calculated using the area of the rubies' incircles (see \Cref{th:square-twincircle-area}).
\end{proof}
...
\begin{proof}
    ...
    The tightness of these bounds for circles was shown in \Cref{th:square-twincircle-area}. For the same reason, two octagons with larger incircles can not be packed: Their incircles would overlap, which means that the octagons themselves would overlap. For squares, the fact that the largest two squares that can be packed into a square (with rotation) are two quarter-squares was shown by \textcite{gobel1979geometrical}.
\end{proof}
...
\fig[2.5]{cover}{A isoscles right triangle can always be covered by two circles with a combined area of its excircle.}
