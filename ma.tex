...
\begin{proof}
    ...
    \begin{figure}[htbp!]
        ...
        \end{tikzpicture}
        \begin{tikzpicture}[scale=1.3]
            \hatsinhat{\defaulta}{\defaultb}{0.8}{0}
        \end{tikzpicture}
        \begin{tikzpicture}[scale=1.3]
    ...
    \begin{itemize}
        \item[(1)]
            ...
            We then observe that the two hats which we want to pack don't overlap if their incircles don't overlap. If we surround the incircles with triangles similar to the container's triangle, like in \Cref{fig:hatsoverlap}, we can see that they don't overlap if $lf + rg \le w$ holds.
        ...
        \item[(2)]
            ...
            So let us assume $a_i \ge f_i$. For this proof, normalize the container hat's incircle to 1. Again, for triangles similar to the container hat's right side, the ratio between the square root of the incircle's area and the longest side is constant, call this constant $d$, see \Cref{fig:hatpokef}.
            ...
            Also, we are interested in the ratio between the square root of the incircle's area of a triangle similar to the container triangle and the length of its right side. We call this ratio $e$. In \Cref{fig:hatpokef} we can observe that
            ...
            %We can tell from \Cref{fig:hatpoke} that this does not happen if the width of the hat's triangle ($d\sqrt{a_i}$), minus the distance between the triangle's upper corner and the tangent to $y$ (...) is at most the length of the container triangle's right side ($d\sqrt{f_i}$).
